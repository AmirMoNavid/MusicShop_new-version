...
const TrackPlayer = ({ playTrack, setPlayTrack }) => {
  const { tracks } = useSelector((state) => state.tracks);

  const location = useLocation();
  const id = location.pathname.split("/")[2];
  const trackLink = tracks.find((t) => t.id === id);
  const { track } = useParams();
  const [show, setShow] = useState(false);
  const [song, setSong] = useState(new Audio(trackLink.url));
  ...
  useEffect(() => {
    const id = location.pathname.split("/")[2];
    const trackLink = tracks.find((t) => t.id === id);
    setSong(new Audio(trackLink.url));
  }, [location]);
  ...
  return (
    ...
    >
      ...
      <div className="z-50 flex h-full w-full flex-col items-start justify-start gap-8 lg:w-[35%]">
        <div className="flex h-full w-full flex-col items-center justify-between gap-2 lg:gap-7">
          <div
            onClick={() => {
              setShow(false);
              song.load();
